...
### `cortex/index.md` and retrieval

`cortex/index.md` is the piece that makes retrieval work without an embedding index: the agent reads the
...
transcripts and append-only logs outrank short canonical notes on keyword match. So `cortex/raw/`,
`brain/log.md` and `cortex/06_Sessions/` are excluded from default search and read only when the question
...
- **One register, one home.** Every assumption's full reasoning lives in
  `cortex/03_Resources/Assumptions.md` and nowhere else; notes carry a one-line pointer. Copies drift, and
...
concludes something worth keeping, and it lives in `cortex/03_Resources/` next to `[[About me]]` for the
...
The shape to notice: `cortex/03_Resources/` is where the value accumulates, `cortex/01_Projects/` and
`cortex/02_Areas/` stay small, and `cortex/00_Inbox/` hovers near zero because `maintain` drains it. If your
