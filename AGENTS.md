...
not language: `brief`, `capture`, a slash command — these are tokens, so "capture — droppet køen,
cron holder" is a Norwegian message and gets a Norwegian answer. And a bare command with no prose
at all ("/brief" as the first message of a session) mirrors nothing — fall back to the language
...
### The friction log — a declared record, kept out of retrieval

One record extends "something they *said*" beyond the current session, and this paragraph is where
it is said out loud. On Claude Code, `brain/bin/friction` (a deterministic `UserPromptSubmit`
hook) watches each prompt for the magic words the human declared in `[[How we work together]]` —
read from the note at runtime, never hardcoded — and appends `timestamp · word · session pointer`
to `cortex/raw/friction.log`. Never the prompt text, never the reply. The log is gitignored and,
like all of `cortex/raw/`, outside the retrieval path: raw material for a later distillation pass
(`ingest-sessions`, `maintain`), whose conclusions reach the vault only as proposals under
[You may propose. Only they may accept.](#you-may-propose-only-they-may-accept) Deleting the log
is the human's right and breaks nothing.
...
| `ingest-sessions` | `brain/prompts/ingest-sessions.md` | Distil the human's AI coding sessions into session notes they can search; a second pass, on request, distils how they work with an AI into proposals — it writes nothing itself. **Reads outside the vault** — see the exceptions above |
...
| `mirror` | `brain/prompts/mirror.md` | Render everything the brain holds about the human — facts with dates, open assumptions, refuted ones, superseded preferences — each line naming its note. Strictly read-only: writes nothing, proposes nothing |
...
| `review-assumptions` | "what have you guessed about me", "let me go through those assumptions" — a tap-fast verdict pass over the register, five at most, verdicts only | Anything the register doesn't hold — perishable follow-ups, blank dimensions, stalled work — `interview`. Raising a new one — `infer`. Reading the model back without ruling on it — `mirror`. |
| `mirror` | "what do you know about me", "show me what you've learned", "what's your picture of me" — the vault's model of them rendered back: facts with the date each was learned, open guesses, refuted ones, replaced preferences, every line naming its note | Ruling on the guesses — `review-assumptions`, which owns verdicts. Filling the gaps by asking them questions — `interview` ("what don't you know about me"). A question about a topic in their notes — `ask`. Changing anything it shows — this command is read-only by construction; the commands that write are named in its close. |
